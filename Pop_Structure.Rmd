...
example, we will use the *microbov* data set. This contains information on 704
cattle from both Africa and France over several different breeds. We can create
a dendrogram over all 704 samples, but that would be difficult to visualize. For
our purposes, let's take ten random samples and calculate Provesti's distance,
which will return the fraction of the number of differences between samples:
...
their country of origin and we are seeing that the tree seems to separate based
off of this. Of course, a tree is a hypothesis and one way of generating support
is to bootstrap loci. This can be achieved with the *poppr* function `aboot`.
...
would be better to create a bootstrapped dendrogram based off of a population
...
make a tree using `aboot` with Nei's genetic distance.
...
Now we can see that, in all 1000 bootstrapped trees, the African and French 
...
of the different analyses for which you can utilize genetic distance:
 ...
 | Minimum Spanning Networks | `poppr.msn` | *poppr* | cannot handle genpop | 
...
different than other distances, so it has its own function for 
...
```{r}
setpop(monpop) <- ~Tree
t26 <- popsub(monpop, "26") %>% setpop(~Year/Symptom)
t26
```
...
PC stands for principal component, are unitless transofrmations of your data
that explaining the variance observed. For the purposes of `find.clusters`, we
...
Notice here that there is no uptick in the curve. This indicates that there
...
Not all methods of clustering need to be based on Bruvos's distance. There are
others available. See `?nei.dist` for more details. The function `aboot` will
perform bootstrap analysis on any distance given for individuals or populations,
and `poppr.msn` will create minimum spanning networks given a genind object and
a distance.
