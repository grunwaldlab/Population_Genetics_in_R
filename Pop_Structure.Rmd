...
(e.g. $G_{ST}$), clustering, or graphing of minimum spanning networks (MSN). Note, that
this chapter will utilize many data sets due to the unique features offered by
each. Let's first look at an example of population differentiation based on $G_{ST}$.
...
these diversity measures, an index of $G_{ST}$ = 0 indicates no differentiation, whereas $G_{ST}$ = 1
...
their country of origin and we are seeing that the populations cluster correspondingly. Of course, a tree is a hypothesis and one way of generating support
