...
To transform the data frame we use `pivot_longer` from the package `tidyr`.
...
`ggplot2` will plot the `dapc.results` data frame we reorganized using `pivot_longer`, using the samples on the X-axis and membership probabilities on the Y-axis. 
