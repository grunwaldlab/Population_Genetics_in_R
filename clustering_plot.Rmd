...
multi-panel ggplot.
Each panel will be a different gglot object, so we'll have to give them unique 
...
`set.seed()` call out to explore how sensitivee your results are to the seed.
...
Here we'll use "facets" to seprate the different values of *K*.
...
The group names in teh assignment test are arbitrary.
This means that what is group 1 in the K = 2 facet may be very similar to group
4 in the K = 3 facet.
At the present I handle this with a manual step.
