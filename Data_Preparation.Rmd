...
over 13 haploid loci `r citep(bib["everhart2014finescale"])`. The data set is
called **monpop.csv**.
...
Being based off of *adegenet*, users can import data from the following formats
...
We will continue discussing the GenAlEx format below.
...
Below is what the monpop.csv data look like. Highlighted in red is
...
represented at each locus. This is coded such that each allele is in a separate
column so that your genetic data will take up 2 $\times$ (number of loci)
columns. Below is an example of the `nancycats` data set (from the *adegenet*
