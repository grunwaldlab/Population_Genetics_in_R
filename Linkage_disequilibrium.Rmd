...
`r citep(bib["goss2014irish"])`. We will use the index of association to give
support to they hypothesis that Mexico is the putative origin of *P. infestans*.
...
Indeed we observe 43 multilocus genotypes out of 48 samples. We are looking ats
partial clonality and thus need to use clone-corrected (also called clone-
censored) data:

```{r, eval=FALSE}
MX %>% clonecorrect(hier = ~Continent/Country) %>% ia(sample = 999)
```
```{r, MXiacc, echo=FALSE}
cat("|================================================================| 100%\n")
set.seed(20154)
MX %>% clonecorrect(hier = ~Continent/Country) %>% ia(sample = 999, quiet = TRUE)
```
...
```{r, eval=FALSE}
SA %>% clonecorrect(hier = ~Continent/Country) %>% ia(sample=999)
```
```{r, SAiacc, echo=FALSE}
cat("|================================================================| 100%\n")
set.seed(9999)
SA %>% clonecorrect(hier = ~Continent/Country) %>% ia(sample=999, quiet=TRUE)
```
