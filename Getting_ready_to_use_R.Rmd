...
Various chapters throughout this primer will have this symbol in the code: `%>%`.
This is called a "pipe" operator and it allows code to be more readable by 
stringing together commands from right to left. When reading code, it can be 
thought of as equivalent to saying "and then". For example, if you have three
steps to a process, you would write in English as:
...
Below, are two examples of how code can be improved with *magrittr*. 
[More details about magrittr can be found in this link.](http://cran.r-project.org/web/packages/magrittr/vignettes/magrittr.html)
...
*Phytophthora infestans* data from North and South America 
`r citep(bib['goss2014irish'])`. Calculating allelic diversity per population
after clone-correction. This information can be found in our chapters on
[Population hierarchies and clone correction](Population_Hierarchies.html) and
...
allelic_diversity <- lapply(seppop(clonecorrect(Pinf, hier = ~Continent/Country)), FUN = locus_table, info = FALSE)

# VS

allelic_diversity <- Pinf %>%
  clonecorrect(hier = ~Continent/Country) %>% # clone censor by continent and country.
...
The `%>%` operator is good if you have to do a lot of small things to the data
in your analysis. It allows your code to be more readable and reproducible. 
...
Next, consider browsing the Appendix 3 on Introduction to R if you are not yet
