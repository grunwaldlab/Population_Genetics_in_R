...
- the dataset H3N2
- `var.contrib` when set to FALSE, flag removes contribution from original
  variables from output. We will look at this later to see which loci are
  responsible for separating populations.
- `n.pca` is the number of axes retained in the Principal Component Analysis
  (PCA). If set to NULL selection is interactive, which we avoid here but can be
  explored by the user.
- `n.da` is the  number of axes retained in the Discriminant Analysis (DA). If
  set to NULL selection is interactive, which we avoid here but can be explored
  by the user.
...
drastically influence an epidemic. It, however, might be a mystery as to how the
number of principal components (PC) was chosen for the analysis. It is important
to carefully choose the correct number of PC because if you choose too few, you
lose resolution, but if you choose too many, you end up over-fitting your data.
One way of ensuring that you have selected the correct number of PC is to do
cross-validation. This is a procedure in which you leave out a certain
percentage of your data, run DAPC, and then see if the data that was left out is
correctly placed into the population.
...
the "Hunter Creek (HunterCr)" population was
shown to be the result of a second introduction, likely from nurseries. Part of
the evidence to support this conclusion was from DAPC results. Here, we will
recreate the process of cross validation and reporting.
...
We also have a DAPC object there. We can plot that, too. Compare to figure 4
of `r citet(bib["kamvar2015spatial"])`
```{r}
...
Please refer to the [vignette](http://adegenet.r-forge.r-project.org/files
/tutorial-dapc.pdf) written by Thibaut Jombart for a more interactive analysis.
