...
will again use the nancycats data to test for HWE using genind objects, a
dataframe particular to the *adegenet* R package:
...
The full report (`nanhwe.full`) will give us a list of loci, each containing a
list of populations, where each population is the result of a $\chi^2$ test. We
will analyze data for a few loci to avoid writing a large amount of data to the
screen. With the following statements, we can see that at locus1 (`fca8`), we
have no good evidence to reject the null hypothesis that population 1 (`P01`) is
not in HWE, but we have evidence that population 2 (`P02`) is not in HWE:
...
Of course, checking all of these is tedious, but we can look at a matrix for all
loci by population:
...
However, this is still hard to sift through. We will thus define an $\alpha$
value and set everything above that value to 1 so that we can visually detect
candidate loci where we might reject the $H_o$ of HWE.
