...
differentiated locally, regionally or globally or in time.  Thus, a combination
...
The methods implemented in *poppr* allow specification of which hierarchies you
...
has two items, the hierarchical levels and the populations defined.  You can
think of hierarchical levels as the index names for each of the hierarchical
levels within your data (so for our data it should be Tree, Year, and Symptom).
By default, however, no hierarchical levels are defined and so this is "Pop",
...
----
We imported the data that has three hierarchical levels "Tree\_Year\_Symptom".
In order to analyze our data according to any combination of those three
hierarchical levels, we need to tell *poppr* that the 12 groups should be split
by tree, year, and/or symptom.  Thus, the first step is to split our data
according to hierarchy so that we can access each of the three hierarchical
levels in the data.  The `splithierarchy` command is used to index the three
hierarchical levels:
...
Now that we have laid out the basics of manipulating populations by hierarchies,
we will now talk a little bit about using these hierarchies for clone correction.
...
This technique is commonly used with the analysis of the index of association
and allelic diversity measures since duplicated genotypes have the potential to
bias the analysis. Since we want to only observe unique genotypes per population,
clone correction requires specification of the hierarchical levels at which
...
use the function `clonecorrect` specifying the "Tree/Year" hierarchy:

```{r, clonecorrection}
mcc_TY <- clonecorrect(monpop, hier = ~Tree/Year, keep = 1:2)
mcc_TY
# Don't forget to set the original data set to the same hierarchy.
setpop(monpop) <- ~Tree/Year
```

Now we can compare the diversity of alleles at each locus using Simpson's index
($1-D$) as implemented in the function `locus_table`. We will do this in three
steps:
...
what to do. A function's form looks like this:
...
As an example function, let's write a function that will take the harmonic mean
of a vector of numbers. First, we'll see how to do it in a non-functional way.
...
1/rmean
```
If we didn't want to type all that, we could just write it into a function so
that it can be reused without so much typing:
...
to do that, we will need to have 3 things:
...
This was a brief introduction to the easiest way to create hierarchical levels
and apply them in *poppr* to more rapidly analyze your data. By indexing the
hierarchical levels of your data, you can set the hierarchical level(s) you want
