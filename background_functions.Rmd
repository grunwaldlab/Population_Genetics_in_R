...
Because of this it is generally recommended to avoid `for()` loops in your R code, particularly when performance is an issue.
...
The function `sweep()` is similar to `apply()` in that it iterates over teh rows or columns of a matrix.
